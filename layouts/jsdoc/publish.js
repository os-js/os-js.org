...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
    ...
    function createSubMenus(item) {
      var inner = '';
      var lbls = ['Members', 'Functions'];

      if ( itemHeading === 'Namespaces' || itemHeading === 'Classes' ) {
        ['member', 'function'].forEach(function(k, i) {
    ...
    if (items.length) {
        ...
        items.forEach(function(item) {
            if ( !hasOwnProp.call(item, 'longname') ) {
                ...
                var inner = createSubMenus(item);

                itemsNav += '<li data-name="' + item.longname + '" data-subs="' + String(inner.length > 0) + '">' + linktoFn(item.longname, displayName.replace(/\b(module|event):/g, '')) + '<span class="folder">+</span>' + inner + '</li>';
...
function linktoCustom(longName, name) {
  return linkto(longName);
}
...
function buildNav(members) {
    ...
    nav += buildMemberNav(members.classes, 'Classes', seen, linktoCustom);
    nav += buildMemberNav(members.events, 'Events', seen, linkto);
    nav += buildMemberNav(members.namespaces, 'Namespaces', seen, linktoCustom);
